...
final public class LogKitaSortAlt extends Sort {
	public LogKitaSortAlt(ArrayVisualizer arrayVisualizer) {
		super(arrayVisualizer);
		
		this.setSortListName("Log Kita (Alternate)");
    ...
    private void tailmerge(int[] array, int[] tmp, int a, int m, int b, boolean copy) {
    	if(copy) Writes.arraycopy(array, m, tmp, 0, b-m, 1, true, true);
    	int l = m-1, r = b-m-1;
    	while(l >= a && r >= 0) {
    ...
    private void kitamerge(int[] array, int[] tmp, int x, int i, int ib, int j, int jb, int p, int c, int w, int w1, boolean y) {
    	...
    	int l = i, ln = getAndFree(array, l, x, p, w, w1, c, y), lth = 0, ltt = 1, lc = 0, ld = 0, r = j, mb = ib,
    		rn = getAndFree(array, r, x+j-i, p, w, w1, c, y), rth = 0, rtt = 1, rc = 0, rd = 0, cc = 0,
    	...
    	if(ltt-lth > 0) {
    		...
    		if(ft >= 0)
    			encode(array, ft, x+lt[lth%3]*w, lt[lth%3]);
    		...
    		if(ft >= 0)
    			encode(array, ft, x+rt[rth%3]*w, rt[rth%3]);
    ...
    // O(n W(n)) time, which is actually pretty good
    private void insertRuns(int[] array, int a, int b, int c, int d, int s) {
    	BinaryInsertionSort bi = new BinaryInsertionSort(arrayVisualizer);
    	int i = a;
    	for(; i < b; i += s) {
        	bi.customBinaryInsert(array, i, Math.min(i+s, b), 0.5);
    	}
    	i = c;
    	for(; i < d; i += s) {
        	bi.customBinaryInsert(array, i, Math.min(i+s, d), 0.5);
    	}
    }
    
    private void mergeFours(int[] array, int[] tmp, int a, int b, int c, int d, int s) {
    	b -= (b - a) % s;
    	d -= (d - c) % s;
    	int i = a;
    	for(; i + s < b; i += 4 * s) {
    		merge(array, tmp, i, i+s, Math.min(i+2*s, b), 0, true);
			merge(array, array, Math.min(i+2*s, b), Math.min(i+3*s, b), Math.min(i+4*s, b), i, false);
			tailmerge(array, tmp, i, Math.min(i+2*s, b-2*s), Math.min(i+4*s, b), false);
    	}
    	i = c;
    	for(; i + s < d; i += 4 * s) {
    		merge(array, tmp, i, i+s, Math.min(i+2*s, d), 0, true);
			merge(array, array, Math.min(i+2*s, d), Math.min(i+3*s, d), Math.min(i+4*s, d), i, false);
			tailmerge(array, tmp, i, Math.min(i+2*s, d-2*s), Math.min(i+4*s, d), false);
    	}
    }
    ...
    private void kita(int[] array, int[] tmp, int a, int b, int x, int w, int p, int c, boolean iv) {
    	...
    	if(b < B)
    		tailmerge(array, tmp, a, b, B, true);
    }
    
    public void logkita(int[] array, int a, int b) {
    	int plg = productLog(b-a);
    	int[] aux = Writes.createExternalArray(2*plg);
    	int m = a+(b-a)/2;
    	// quickselect middle
    	int[] bnds = quickselect(array, aux, a, b, m);
    	// prepare both halves
    	insertRuns(array,      a, bnds[0], bnds[1], b, plg);
    	mergeFours(array, aux, a, bnds[0], bnds[1], b, plg);
    	// block merge
    	kita(array, aux, a, bnds[0], bnds[0], plg-1, array[m], 0, true);
    	kita(array, aux, bnds[1], b, a, plg-1, array[m], 1, false);
    }
